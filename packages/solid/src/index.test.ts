...
import { render, solid2Baseline } from './index.js';
...
describe('Solid 2 baseline', () => {
  ...
  it('turns a signal change into one text mutation', () => {
    ...
    try {
      const dispose = render(() => `Count: ${count()}`, host.root);
      flush();

      expect(host.root.children).toHaveLength(1);
      const text = host.root.children[0];
